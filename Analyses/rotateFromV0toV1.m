...
% if vec0(2) < 0
%     theta = acos( dot(vec0, vec1)/norm(vec0)/norm(vec1)); %angle that data needs to be rotate (radians)
% elseif vec0(2) >= 0
%     theta = -acos( dot(vec0, vec1)/norm(vec0)/norm(vec1)); %angle that data needs to be rotate (radians)
% end

% %%Plot unrotated/translated data (for debugging)
% figure(1)
% plot(X, Y,'r')
% hold on
% 
...
% x-rotated = x*cos(theta) + y*sin(theta)
% y-rotated = -x*sin(theta) + y*cos(theta)
