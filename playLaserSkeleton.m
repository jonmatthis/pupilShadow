...
if recordVid
    vidFileName = [w.subID '_' w.takeID '_' w.trialType '_laserSkeleton.mp4'];
...
for fr = frames
    ...
    if fr ==1 %skip the first frame, for reasons
        continue
    end
    ...
    if worldFrameIndex(fr) ~= worldFrameIndex(fr-1)
        thisFrameRaw = worldVidObj.read;
        
        if showOpticFlow
            floH5 = h5read(flowH5path, ['/' num2str(worldFrameIndex(fr))]);
    ...
    plot(porXdisp,'ro-','LineWidth',1,'MarkerSize',1.5);
    hold on
    ...
    plot(porYdisp,'bo-','LineWidth',1,'MarkerSize',1.5);
    hold on
